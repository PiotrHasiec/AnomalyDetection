...
LATTEM_DIM = 4
PATH = ".\\Modele\\B04"
MODEL_NAME = "prediction_model_lr1_t"+str(TIME_STEP)
...
def load_data(datas,timestep = TIME_STEP,number_of_timesteps_in = NUMBER_OF_TIMESTEPS_IN_SERIE_IN,number_of_timesteps_out=NUMBER_OF_TIMESTEPS_IN_SERIE_OUT):
    ...
    timeserie_datas_y = [ timeseries_dataset_from_array(serie[timestep:],None,sequence_stride = number_of_timesteps_in,sequence_length=number_of_timesteps_out,batch_size=1,shuffle=True,seed=1237,) for i,serie in enumerate(datas)]
    x = []
    y = []
    ...
    train_x,test_x = np.array(train_x).reshape(-1,number_of_timesteps_in,len(joints)),np.array(test_x).reshape(-1,number_of_timesteps_in,len(joints))
    train_y,test_y = np.array(train_y).reshape(-1,number_of_timesteps_out,len(joints)),np.array(test_y).reshape(-1,number_of_timesteps_out,len(joints))
